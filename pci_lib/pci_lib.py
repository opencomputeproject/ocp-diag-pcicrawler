...
from pci_vpd_lib.pci_vpd_lib import VitalProductDataReader, VPDDataException
...
PCIExpressSlot = namedtuple("PCIExpressSlot", ("slot", "presence", "power", "attn_led"))
...
class PCIConfigSpace(object):
    ...
    def __init__(self, devname):
        if devname in PCIConfigSpace.configspaces:
            raise Exception(
                "Attempt to open PCI config space twice for device: {}".format(self)
            )
        self.device_name = devname
        self.path = "{}{}/config".format(SYSFS_PCI_BUS_DEVICES, self.device_name)
        self.fd = None
        self.open()
    ...
    def __repr__(self):
        return "PCIConfigSpace('{}', sz={})".format(self.device_name, self.size)
...
class PCIDevice(
    ...
    def vpd(self):
        ...
        if os.path.exists(vpdfile):
            try:
                ...
                return {
                    "identifier_string": vpdreader.identifier_string,
    ...
    def __repr__(self):
        return (
            "<PCIDevice vendor_id=0x{p.vendor_id:04X} "
            ...
            "device_function_id=0x{p.device_function_id:04X}>"
        ).format(p=self)
...
def map_pci_device(device_name):
    ...
    return PCIDevice(
        device_name, vendor, device, pci_class, pci_subvendor, pci_subsystem
    )
...
def shorten_pci_addr(pci_addr):
    ...
    if m1:
        if m1.group(1) != "0000":
            raise NonZeroDomain()
        pci_addr = "{}:{}.{}".format(m1.group(2), m1.group(3), m1.group(4))
    elif m2:
        pass
    else:
...
def load_pci_ids():
    ...
    if not pci_ids_location:
        raise RuntimeError("No pci.ids file avail in %r" % pci_ids_locations)
