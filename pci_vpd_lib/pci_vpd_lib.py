    ...
    def _read_vpd(self, f):
        ...
        while tag != 0xF:
            ...
            if tag == 0x02:
                ...
                checksum = self._combine_checksum(checksum, bytearray([header_sum]))
                checksum = self._combine_checksum(checksum, bytearray(value))
                ...
                if len(data) < l:
                    raise VPDDataException("VPD data is truncated!")
                checksum = self._combine_checksum(checksum, bytearray([header_sum]))
                checksum = self._combine_checksum(checksum, bytearray(data))
